...
class Vision(object):
    def __init__(self,field,index,type,guid=0000000000000000,buffersize=10):
        ...
        self._isAgentDrawingEnabled = False   # draw separation vector and COM for agents
        ...
        if self.isFireWire():
            ...
            self.cap = cv2.VideoCapture('arrow_temp.mp4')
        ...
        if self.isFireWire():
            ...
            if self.isUpdating():
                ...
                if not self.isFilterBypassed() and not self.filterRouting == []:
                    ...
                    frameFiltered = frameOriginal
                if self.isObjectDetectionEnabled():
                    frameProcessed, self.args = self.processObjectDetection(frameFiltered,frameOriginal)
                else:
                    frameProcessed = frameFiltered
                if self.isFieldDrawingEnabled():
                    self.fieldDrawing(self._field.x,self._field.y,self._field.freq,self._field.mag,self.time,self.separation,self.orientation)
    ...
    def agentDrawing(self,x,y):
        ...
        if x != 0 and y != 0:
            ...
            self.addDrawing('circle',(self.args[0],self.args[1],2,3))
            self.addDrawing('circle',(self.args[2],self.args[3],2,3))


    def COMDrawing(self,x1,y1):
        self.COMx = x1
        self.COMy = y1
        self.addDrawing('circle',(x1,y1,2,4))
